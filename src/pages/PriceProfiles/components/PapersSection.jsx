...
const PriceField = React.memo(({
  ...
  // Detectar cambios pendientes para este papel específico
  const hasChanged = useMemo(() => {
    const inputValue = parseFloat(paperPriceInputs[paper.id]);
    return !isNaN(inputValue) && inputValue !== paper.pricePerSheet;
  }, [paperPriceInputs, paper.id, paper.pricePerSheet]);
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-200 transition-all duration-200 hover:border-gray-300">
      ...
      <input
        ...
        disabled={loadingAll}
        className={`w-full p-3 border-2 rounded-lg transition-all duration-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed ${
          hasChanged
      ...
      <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
        ...
        Actual: ${paper.pricePerSheet?.toFixed(2) || "0.00"} /pliego
      </p>
    </div>
  );
});
...
/**
 * Sección para gestionar precios de papel con UX mejorada:
    ...
    <section className={`${colors.bg} p-4 sm:p-6 rounded-xl shadow-md`}>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border-2 border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-5">
          ...
          <button
            onClick={updateAllPaperPrices}
            disabled={loadingAll || !hasAnyChanges}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {papers.map((paper) => (
            <PriceField
              key={paper.id}
              paper={paper}
              paperPriceInputs={paperPriceInputs}
              handlePaperPriceInputChange={handlePaperPriceInputChange}
